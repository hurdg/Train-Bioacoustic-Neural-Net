...
# Train the model...
    ...
    # ----------------------------------------------------------------
        ...
        with torch.no_grad():
            ...
            print(f'''Val Accuracy {val_acc.item():.4f}, Val Precision: {val_prc.item():.4f}, Val Recall: {val_rec.item():.4f} \n''')
        ...
        val_accuracy=val_acc, val_precision = val_prc, val_recall = val_rec, val_loss = avg_val_loss))
        
        #Stop model if val_loss hasn't decrease by more than X within the last X epochs.
        early_stopping(avg_val_loss, val_acc, val_prc, val_rec)
